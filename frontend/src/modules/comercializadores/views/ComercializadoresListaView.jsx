...
const ComercializadoresListaView = () => {
  ...
  const { data: comercializadores, loading, error, refetch } =
    useFetch("/comercializadores");
  ...
  const [modalEditarComercializadorId, setModalEditarComercializadorId] =
    useState(null);

  const [paginaActual, setPaginaActual] = useState(1);
  const PAGE_SIZE = 10;
  const totalPaginas = comercializadores
    ? Math.ceil(comercializadores.length / PAGE_SIZE)
    : 0;

...
import { getCentroDetalleCompleto } from "../services/centros_apuesta.service";
...
const CentrosApuestaDetalleModal = ({ idCentro, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const res = await getCentroDetalleCompleto(idCentro);
        setData(res);
      } catch (err) {
        setError(
          err.response?.data?.message ||
            "Error al cargar el detalle del centro de apuesta",
        );
      } finally {
        setLoading(false);
      }
      ...
      <CModalBody>
        ...
        {!loading && !error && data && (
          <div className="px-2">
            ...
            <h5 className="section-title">Información del Centro</h5>
            <CRow className="gy-3 mb-2">
              <Campo
                label="Nombre de la Agencia"
                value={data.nombre_agencia}
                bold
                md={6}
              />
              <CCol md={6}>
              ...
              <Campo label="Dirección" value={data.direccion} md={12} />
            </CRow>
            ...
            <CRow className="gy-3 mb-2">
              <Campo
                label="Razón Social"
                value={data.comercializador_razon_social}
                bold
                md={12}
              />
              <Campo
                label="RIF"
                value={data.comercializador_rif}
                md={6}
              />
              <Campo
            ...
            {representantes.length > 0 ? (
              ...
              <CAlert color="info" className="py-2 small mb-0">
                Este centro de apuesta no tiene representantes legales
                registrados.
              </CAlert>
            )}
          </div>

...
const BancosDetalleModal = ({ idBanco, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setError(err.response?.data?.message || 'Error al cargar el detalle del banco')
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <CModal visible={!!idBanco} onClose={onClose} alignment="center" size="lg" backdrop="static">
      <CModalHeader>
      ...
      <CModalBody>
        ...
        {!loading && !error && data && (
          <div className="px-2">
            <h5 className="text-primary fw-semibold mb-3">Información del Banco</h5>
            <CRow className="gy-3 mb-4">
              <CCol md={6}>
                <CFormLabel className="text-muted small fw-semibold mb-1">Nombre</CFormLabel>
                <CFormInput type="text" value={data.nombre || ''} readOnly className="bg-light fw-semibold" />
              </CCol>
              <CCol md={6}>
                <CFormLabel className="text-muted small fw-semibold mb-1">Codigo BCV</CFormLabel>
                <CFormInput type="text" value={data.codigo || 'No registrado'} readOnly className="bg-light" />
              </CCol>
              <CCol md={6}>

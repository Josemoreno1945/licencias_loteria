...
const base_autorizacion_especial_schema = z.object({
  ...
  nro_mesa: z
    .number({ invalid_type_error: "El numero de mesa debe ser numérico" })
  ...
  tipo: z
    .enum(["Movil", "Localidad", "Mesa"], {
      required_error: "El tipo de autorizacion especial es requerido",
    })
    .default("Mesa"),
  ...
  direccion_centro_asignado: z
    .string()
    .max(500, "La direccion no puede exceder los 500 caracteres")
    .optional()
    .nullable(),

  direccion_localidad: z
    .string()
    .max(500, "La direccion no puede exceder los 500 caracteres")
    .optional()
    .nullable(),

  direccion_responsable: z
    .string()
    .max(500, "La direccion no puede exceder los 500 caracteres")
    .optional()
    .nullable(),
...
export const actualizar_autorizacion_especial_schema = base_autorizacion_especial_schema
  .omit({ id_documento: true })
  .partial();
 ...
 * nro_mesa es obligatorio solo cuando tipo = 'Mesa'.
 */
export const emitir_autorizacion_especial_schema = z
  .object({
  ...
  // Si el tipo es Mesa, el número de mesa es obligatorio
  .refine(
    (data) => data.tipo !== "Mesa" || (data.tipo === "Mesa" && data.nro_mesa != null),
    {
      path: ["nro_mesa"],
      message: "El numero de mesa es requerido cuando el tipo es Mesa",
    },
  );
